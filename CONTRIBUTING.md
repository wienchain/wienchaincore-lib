# Contributing to wienchaincore-lib

We're working hard to make *wienchaincore-lib* the most powerful JavaScript library for working with Wienchain. Our goal is to have *wienchaincore-lib* be a library that can be used by anyone interested in Wienchain, and to level expertise differences with great design and documentation.

## Community

If there are any questions, etc., please feel to ask in one of the community channels:

- https://www.wienchain.com 
...
These are some global design goals in wienchaincore-lib that any change must adhere.
...
When possible, wienchaincore-lib objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of wienchaincore-lib methods. For example, calling `JSON.stringify` on an wienchaincore-lib object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'wienchaincore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the wienchaincore object.

DO:
```javascript
var wienchaincore = require('../');
var PublicKey = wienchaincore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add wienchain git@github.com:wienchain/wienchaincore-lib.git
git pull --rebase wienchain master
```
...
```
Finally go to [github.com/wienchain/wienchaincore-lib](https://github.com/wienchain/wienchaincore-lib) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of wienchaincore-lib.

If you have any questions feel free to post them to
[github.com/wienchain/wienchaincore-lib/issues](https://github.com/wienchain/wienchaincore-lib/issues).
