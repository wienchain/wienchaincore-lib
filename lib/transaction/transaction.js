...
Transaction.prototype.checkedSerialize = function(opts) {
  ...
  if (serializationError) {
    serializationError.message += ' - For more information please see: ' +
      'https://wienchaincore.io/api/lib/transaction#serialization-checks';
...
Transaction.prototype._hasFeeError = function(opts, unspent) {
  ...
  if (!opts.disableLargeFees) {
    var directSendFee = this.estimateDirectSendFee();
    var maxFeePerNormalTransaction = Math.floor(Transaction.FEE_SECURITY_MARGIN * this._estimateFee());
    var maximumFee = Math.max(maxFeePerNormalTransaction, directSendFee);
 ...
 * @return {number}
 */
Transaction.prototype.estimateDirectSendFee = function estimateDirectSendFee() {
  return this.inputs.length * constants.DIRECTSEND_FEE_PER_INPUT;
};
 ...
 * and a slightly fancier format recognized by wienchaincore:
 *
 * ```
 * {
 ...
 * }
 * ```
 * Where `address` can be either a string or a wienchaincore Address object. The
 * same is true for `script`, which can be a string or a wienchaincore Script.
 *
